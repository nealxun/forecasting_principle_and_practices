...
yr <- 2012:2016
knitr::kable(data.frame(Year=yr,Observation=x))
```
...
```{r fig.cap="Weekly economy passenger load on Ansett Airlines.", cache=TRUE}
...
-   A *trend* exists when there is a long-term increase or decrease in the data. There is a trend in the antidiabetic drug sales data shown in Figure \@ref(fig:a10).
-   A *seasonal* pattern occurs when a time series is affected by seasonal factors such as the time of the year or the day of the week. The monthly sales of antidiabetic drugs above shows seasonality which is induced partly by the change in the cost of the drugs at the end of the calendar year.
-   A *cycle* occurs when the data exhibit rises and falls that are not of a fixed period. These fluctuations are usually due to economic conditions, and are often related to the "business cycle". The time series in Figure \@ref(fig:hsales) shows both seasonality (the annual pattern) and cyclicity (the rises and falls lasting several years).

```{r hsales, echo=TRUE, cache=TRUE, fig.cap="Monthly sales of new one-family houses sold in the USA, 1973-1995."}
autoplot(hsales) +
  ggtitle("Sales of new one-family houses, USA") +
  xlab("Year") + ylab("Total sales")
```

It is important to distinguish cyclic patterns from seasonal patterns. Seasonal patterns have fixed and known lengths, while cyclic patterns have variable and unknown lengths. The average length of a cycle is usually longer than that of seasonality, and the magnitude of cyclic variation is usually more variable than that of seasonal variation. Cycles and seasonality are discussed further in Section \@ref(sec-6-1-TSpatterns).
...
```{r, fig.cap="Seasonal plot of monthly antidiabetic drug sales in Australia.", out.width="90%", cache=TRUE}
...
```{r, fig.cap="Seasonal subseries plot of monthly antidiabetic drug sales in Australia.", cache=TRUE}
...
```{r beeracf, fig.cap="Autocorrelation function of quarterly beer production.", cache=TRUE}
ggAcf(beer2)
```
...
set.seed(30)
y <- ts(rnorm(50))
autoplot(y, main="White noise")
```

```{r wnoiseacf, fig.cap="Autocorrelation function for the white noise series.", cache=TRUE}
ggAcf(y)
```

For white noise series, we expect each autocorrelation to be close to zero. Of course, they will not be exactly equal to zero as there is some random variation. For a white noise series, we expect 95% of the spikes in the ACF to lie within $\pm 2/\sqrt{T}$ where $T$ is the length of the time series. It is common to plot these bounds on a graph of the ACF (the blue dashed lines above, and also shown in the ACF plot for the beer data). If one or more large spikes are outside these bounds, or if substantially more than 5% of spikes are outside these bounds, then the series is probably not white noise.
...
1. Download some monthly Australian retail data from [http://robjhyndman.com/data/retail.xlsx](http://robjhyndman.com/data/retail.xlsx). These represent retail sales in various categories for different Australian states, and are stored in a MS-Excel file.
    ...
    c. Explore your chosen retail time series using the following functions:
        ...
        `autoplot`, `ggseasonplot`, `ggmonthplot`, `gglagplot`, `ggAcf`
...
3. The following time plots and ACF plots correspond to four different time series. Your task is to match each  time plot in the first row with one of the ACF plots in the second row.
