...
```{r beeracf, fig.cap="Autocorrelation function of quarterly beer production.", fig.height=3}
ggAcf(beer2)
```
...
```{r acfelec, echo=TRUE, fig.cap="ACF of monthly Australian electricity demand.", fig.height=3}
ggAcf(aelec, lag=48)
```
...
```{r wnoiseacf, fig.cap="Autocorrelation function for the white noise series.", fig.height=3}
ggAcf(y)
```
...
4. The following time plots and ACF plots correspond to four different time series. Your task is to match each  time plot in the first row with one of the ACF plots in the second row.
    ...
    ```{r acfguess, fig.height=6, fig.width=12, echo=FALSE, message=FALSE, warning=FALSE}
