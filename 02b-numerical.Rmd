...
pander::pander(temptable)
```

In this example, $N=16$ and $x_{i}$ denotes the maximum temperature in year $i$. Then the average carbon footprint is
\begin{align*}
\bar{x} & = \frac{1}{16}\sum_{i=1}^{16} x_{i} \\
        & = (x_{1} + x_{2} + x_{3} + \dots + x_{16})/16 \\
        & = (38.8 + 44.3 + 43.0 + \dots + 39.7 + 43.7 + 42.9)/16 \\
        & = 668.1/16  = 41.8 \text{ degrees Celsius}.
\end{align*}
...
$$ 
 \text{median} = (41.5+42.7)/2 = 42.1.
$$

*Percentiles* are useful for describing the distribution of data. For example, 90% of the data are no larger than the 90th percentile. In the maximum temperature example, the 90th percentile is 44.1 because 90% of the data (14 observations) are less than or equal to 44.1. Similarly, the 75th percentile is 43.25 and the 25th percentile is 39.70. The median is the 50th percentile.
...
$$ 
  s = \sqrt{\frac{1}{19} \left[ (38.8-42.1)^2 + (44.3 - 42.1)^2 + 
          \cdots + (42.9-42.1)^2\right]} = 2.52. 
$$
...
For example, the correlation between the carbon footprint and city mpg variables shown in Figure [2.5](http://www.otexts.org/fpp/2/1#fuel.png) is -0.97. The value is negative because the carbon footprint decreases as the city mpg increases. While a value of -0.97 is very high, the relationship is even stronger than that number suggests due to its nonlinear nature.
