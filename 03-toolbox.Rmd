...
y_t =
  \begin{cases}
     \exp(w_t)\left[1 + \frac{\sigma^2}{2}\right] & \text{if $\lambda=0$;}\\
     (\lambda w_t+1)^{1/\lambda}\left[1 + \frac{\sigma^2(1-\lambda)}{2(\lambda w_t+1)^{2}}\right] & \text{otherwise;}
  \end{cases}
\end{equation}
where $\sigma^2$ is the forecast variance. The larger the forecast variance, the bigger the difference between the mean and the median.
...
The `window` function introduced in Chapter \@ref(ch-graphics) is very useful when extracting a portion of a time series. Another useful function is `subset` from the `forecast` package which allows for more types of subsetting. A great advantage of this functions is that it allows to use indices to choose a subset. For example,
...
```
extracts all the first quarters.
...
`We say here WHEN COMPUTED - did you have something in mind or should be change this to simply say - A forecast error is the difference between the observed value and the forecast using the training data:` 
...
Note that forecast errors are different from residuals in two ways. Firstly, residuals are calculated on the training set while forecast errors are calculated on the test set. Secondly residuals are based on one-step forecasts while forecast errors can involve multi-step forecasts.
...
In this book, we will use the pipe operator whenever it makes the code easier to read. In order to be consistent, we will always follow a function with parentheses to differentiate it from other objects, even if it has no arguments. See for example the use of `sqrt()` in the code above. 


###Example: using tsCV() {-}
` We could do this with the dj - I have done it below - another idea is to replace all dj by goog??`
...
```
The code below evaluates the foresting performance of $1$ to $8$-steps-ahead `naive` forecasts with `tsCV` using MSE as the forecast error measure. The plot shows that the forecast error increases as the forecast horizon increases which we would naturally expect. 
...
e <- matrix(NA_real_, nrow = length(dj), ncol = 8)
for (h in 1:8)
  e[, h] <- tsCV(dj, forecastfunction = naive, h = h)
