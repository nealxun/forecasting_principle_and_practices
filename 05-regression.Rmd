...
Using the `checkresiduals` function introduced in Section \@ref(toolbox-resids) we can obtain all the useful residual diagnostics mentioned above. Figure \@ref(fig:beerresidcheck) shows a time plot, the ACF and the histogram of the residuals from the model fitted to the beer production data as well as the  Breusch-Godfrey test for testing up to 8th order autocorrelation.
...
knitr::kable(tbl, booktabs=TRUE, align='c')
```
