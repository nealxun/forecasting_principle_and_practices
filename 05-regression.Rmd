...
<!--
```{r ScatterMatrixold, echo=TRUE, cache=TRUE, fig.cap="A scatterplot matrix of US consumption expenditure and the four predictors."}
...
# Rob: Because they are asymmetric - reversing the axes gives a different line,
#  unlike correlations. Also, the slopes do not correspond to the parameters of the
#  multiple regression equation which is confusing to students.
```
-->
...
Using the `checkresiduals` function introduced in Section \@ref(toolbox-resids), we can obtain all the useful residual diagnostics mentioned above. Figure \@ref(fig:beerresidcheck) shows a time plot, the ACF and the histogram of the residuals from the model fitted to the beer production data as well as the Breusch-Godfrey test for testing up to 8th order autocorrelation. (The `checkresiduals` function will use the Breusch-Godfrey test for regression models, but the Ljung-Box test otherwise.)
