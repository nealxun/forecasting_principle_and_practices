...
Figure \@ref(fig:MultiPredictors) shows additional predictors that may be useful for forecasting US consumption expenditure. These are quarterly percentage changes of industrial production and personal savings, and quarterly changes in the unemployment rate (as this is already a percentage). Building a multiple linear regression model can potentially generate more accurate forecasts as we expect consumption expenditure to not only depend on personal income but other predictors as well. 


```{r MultiPredictors, echo=TRUE, cache=TRUE, fig.cap="Quarterly percentage changes of industrial production and personal savings and quarterly changes in the unemployment rate for the US."}
...
The first column of Figure \@ref(fig:ScatterMatrix) shows the relationships between consumption expenditure and each predictor. The fitted regression lines show positive relationships for consumption wiht each income and industrial production and negative relationships with each unemployment and savings. The strength of these relationships are shown by the correlation coefficients across the first row.
...
y_t=\beta_0 + \beta_1 x_{1,t}+ \beta_2 x_{2,t}+ \beta_3 x_{3,t}+ \beta_4 x_{4,t}+\varepsilon_t,
 (\#eq:usMLR)
\end{equation}
where $y$ is the percentage change in real personal consumption expenditure, $x_1$ is the and real personal disposable income ( xx ) for the US from the first quarter of 1960 to the third quarter of 2016.
