...
The advantage of the regression models we have so far considered is that they can be used to capture provide vitally important analysis of the relationships between tourism demand and economic variables such as prices, interest rates, and output, among others (see, e.g., Crouch 1992; Li, Song, and
Witt 2005; Song and Li 2008). 

A major challenge with the regression models we have so far considered is that to forecast $y$ the model requires future values of $x$. If scenario based forecasting is of interest then these models are extremely useful. 

If this is not the case then forecasts of the predictors need to be generated and fed to the model. As mentioned above in this case forecasts for the predictors can be generated using pure times series models (introduced later). However, in many cases it can be more challening to produce forecasts for the predictors using a pure time series model than to directly predict the forecast variable. 


the disadvantage with the usual econometric causal models when forecasting is that one has to first forecast the economic variables themselves before forecasting tourism demand, the variable of interest.

In particular, we build what we refer to as “predictive regressions” that use as predictors lags of the commonly used economic variables
and therefore forego the challenge of these needing to be
forecast.

Assuming that we are interested in forecasting $h$-steps ahead we can write
$$y_{t+h}=f(x_t)+\varepsilon_{t+h}$$ for $h=1,2\ldots$.
