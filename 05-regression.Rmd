...
## The linear model {#sec-4-1-Intro}
...
An example of data from such a model is shown in Figure \@ref(fig:SLRpop1). The parameters $\beta_0$ and $\beta_1$ denote the intercept and the slope of the line respectively. The intercept $\beta_0$ represents the predicted value of $y$ when $x=0$. The slope $\beta_1$ represents the predicted increase in $y$ resulting from a one unit increase in $x$.
...
```{r ConsInc, echo=TRUE, cache=TRUE, fig.cap="Percentage changes in personal consumption expenditure for the US."}
autoplot(usconsumption, ylab="", xlab="Year", facet=TRUE) 
```

Figure \@ref(fig:ConsInc) shows time series plots of quarterly percentage changes (growth rates) of real personal consumption expenditure ($y$) and real personal disposable income ($x$) for the US for the period March 1970 to Dec 2010. 

```{r ConsInc2, echo=TRUE, cache=TRUE, fig.cap="Plot"}
fit.ex3 <- tslm(consumption ~ income, data=usconsumption) 
ggplot(as.data.frame(usconsumption), aes(x=income, y=consumption)) + geom_point() + 
...
$$
  \hat{y}=`r coef(fit.ex3)[1]` + `r coef(fit.ex3)[2]`x.
$$
The estimate results are given below.

```{r ConsInc3, echo=TRUE, cache=TRUE}
summary(fit.ex3)
```

(We will show how this line can be estimated in Section \@ref().

This equation shows that a value of 1 for $x$ (the percentage increase in personal disposable income) will result in a forecast value of $0.52+0.32\times1 = 0.84$ for $y$ (the percentage increase in personal consumption expenditure). When $x$ is zero (there is no change in personal disposal income since the last quarter), $y$ takes the value $0.52$ --- there is an increase in consumption of 0.52%.

In general, the slope coefficient can be interpreted as the effect on $y$ of an increase of 1 in $x$ . 
...
Second, we make these assumptions about the errors $(\varepsilon_{1},\dots,\varepsilon_{T})$:
...
A linear trend can be included by using the predictor $x_{1,t}=t$.
...
For example, suppose we are forecasting daily electricity demand and we want toaccount for the day of the week as a predictor. Then the following dummy variables can be created.
...
The estimated coefficients are used for forecasting. For each set of values for the predictors $x_1,\dots,x_k$ we can forecast a corresponding value of $y$ using 
$$
...
```{r beeragain, echo=TRUE, cache=TRUE, warning=FALSE, message=FALSE}
beer2 <- window(ausbeer, start=1992)
autoplot(beer2)
```

We want to forecast the value of future beer production. There are no other variables available for predictors. Instead, we can use the number of quarters since the start of the series as a predictor variable (the trend). We may also use the quarter of the year corresponding to each observation as a predictor variable (a seasonal dummy variable). Then, knowing the number of quarters since the start of the series and the specific quarter of interest, we can forecast the value of beer production in that quarter.

```{r ausbeerfit, echo=TRUE, cache=TRUE}
fit <- tslm(beer2 ~ trend + season) 
summary(fit)
```

Thus, we model the Australian beer production data using a regression model with a linear trend and quarterly dummy variables: 
\[
...
There is a strong downward trend of `r round(fit$coef['trend'], 3)` megalitres per quarter. On average, the second quarter has production of `r -round(fit$coef['season2'], 1)` megalitres lower than the first quarter, the third quarter has production of `r -round(fit$coef['season3'], 1)` megalitres lower than the first quarter, and the fourth quarter has production `r round(fit$coef['season4'], 1)` megalitres higher than the first quarter. The model explains `r round(summary(fit)$r.squared*100,1)`% of the variation in the beer production data.
...
When this calculation is done using values of the predictors from the data that were used to estimate the model, we call the resulting values of $y$ the "fitted values".  We write these as 
$$
  \hat{y}_t = \hat\beta_{0} + \hat\beta_{1} x_{1,t} + \hat\beta_{2} x_{2,t} + \cdots + \hat\beta_{k} x_{k,t},
$$ 
for $t=1,\dots,T$. Each $\hat{y}_t$ is the forecast corresponding to the observation $y_t$ using the predictor information. These are "predictions" of the data used in estimating the model. They are not genuine forecasts as the actual value of $y$ for that set of predictors was used in estimating the model, and so the value of $\hat{y}$ is affected by the true value of $y$.

The following plots show the actual values compared to the predicted (i.e., fitted) values for the Australian beer data.

```{r beerlm2, echo=TRUE, cache=TRUE, fig.cap="Time plot of beer production and predicted beer production."}
df <- cbind(Data=beer2, fitted=fitted(fit))
...
You should always look at an ACF plot of the residuals. This will reveal if there is any autocorrelation in the residuals (suggesting that there is information that has not been accounted for in the model).
...
```{r beerlm5, echo=TRUE, cache=TRUE, fig.cap="Histogram of residuals from regression model for beer production.", message=FALSE, warning=FALSE}
x <- -43:43
ggplot() + geom_histogram(aes(res), binwidth=7) + 
  xlab("Residuals") + ggtitle("Histogram of residuals") +
  geom_line(aes(x=x, y=length(res)*7*dnorm(x,0,sd(res)), col="red")) +
  geom_rug() + guides(col=FALSE)
```

In this case, the residuals seem to be slightly negatively skewed, although that is partly due to the outlier.
...
More often than not, time series data are "non-stationary"; that is, the values of the time series do not fluctuate around a constant mean or with a constant variance. We will deal with time series stationarity in more detail in Chapter \@ref(ch8), but here we need to address the effect non-stationary data can have on regression models.

```{r spurious, echo=TRUE, cache=TRUE, fig.cap="Trending time series data can appear to be related, as shown in this example in which air passengers in Australia are regressed against rice production in Guinea.", message=FALSE,warning=FALSE}
df <- as.data.frame(cbind(GuineaRiceProduction=guinearice,AustralianPassengers=ausair))
p1 <- autoplot(ausair) + xlab("Year") + ylab("Air Passengers in Australia (millions)")
p2 <- autoplot(guinearice) + xlab("Year") + ylab("Rice Production in Guinea (million tons)")
gridExtra::grid.arrange(p1,p2,ncol=2)
ggplot(df,aes(x=GuineaRiceProduction, y=AustralianPassengers)) + geom_point()
```
...
Regressing non-stationary time series can lead to spurious regressions. High $R^2$s and high residual autocorrelation can be signs of spurious regression. We discuss the issues surrounding non-stationary data and spurious regressions in detail in Chapter \@ref(ch9).
