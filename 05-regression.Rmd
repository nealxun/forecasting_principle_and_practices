...
The advantage of the regression models we have so far considered is that they provide a way to capture and analyse  important relationships between the forecast variable $y$ and the predictors. A major challenge though is that in order to forecast $y$ the model requires future values of $x$ as these variables are contemporaneously related. If scenario based forecasting is of interest then these models are extremely useful. 
