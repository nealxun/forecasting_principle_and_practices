...
fit.consMR <- tslm(Consumption ~ Income+Production+Unemployment+Savings, data=usts)
summary(fit.consMR)
```
...
There is an outlier in the residuals (2004Q4) which suggests there was something unusual happening in that quarter. It would be worth investigating that outlier to see if there were any unusual circumstances or events that may have reduced beer production for the quarter.
...
We would expect the residuals to be randomly scattered without showing any systematic patterns. A simple and quick way for a first check is to examine a scatterplot of the residuals against each of the predictor variables. Figure \@ref(fig:resids) seems to be randomly scattered
...
df <- as.data.frame(cbind(res=residuals(fit.consMR),Income=usts[,"Income"],Production=usts[,"Production"],Savings=usts[,"Savings"],Unemployment=usts[,"Unemployment"]))
