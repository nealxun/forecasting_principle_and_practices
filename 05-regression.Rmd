...
df <- as.data.frame(cbind(Data=uschange[,"Consumption"], Fitted=fitted(fit.consMR)))
...
df <- as.data.frame(cbind(Data=beer2, Fitted=fitted(fit.beer)))
...
df$Residuals <- c(residuals(fit.consMR))
...
df <- as.data.frame(cbind(Fitted=fitted(fit), Residuals=residuals(fit)))
...
As discussed in Section \@ref(accuracy), cross-validation is a very useful way of determining the predictive ability of a model. In general, leave-one-out cross-validation for regression can be carried out using the following steps.
...
newdata=data.frame(cbind(Income=rep(-1,h),Savings=rep(-0.5,h),Unemployment=rep(0,h)))
...
```{r, cache=TRUE, include=FALSE}
h=4
fcast.up <- forecast(fit.cons, newdata=data.frame(Income=rep(mean(uschange[,"Income"]),h))) 
...
5 . *(For advanved readers following from Section \@ref(Regr-MatrixEquations))*. Using matrix notation it was shown that if $\bm{y}=\bm{X}\bm{\beta}+\bm{\varepsilon}$, where $\bm{e}$ has mean $\bm{0}$ and variance matrix $\sigma^2\bm{I}$, then the estimated coefficients are given by $\hat{\bm{\beta}}=(\bm{X}'\bm{X})^{-1}\bm{X}'\bm{y}$ 
the forecasts are given by $\hat{y} = \bm{x}^*\hat{\bm{\beta}}=\bm{x}^*(\bm{X}'\bm{X})^{-1}\bm{X}'\bm{Y}$ where $\bm{x}^*$ is a row vector containing the values of the regressors for the
forecasts (in the same format as X), and the forecast variances are given by
$\sigma^2 \left[1 + \bm{x}^* (\bm{X}'\bm{X})^{-1} (\bm{x}^*)'\right].$ 

In this exercise, you will derive specific forecast expressions for the simple time trend model where $Y_t = a + bt$. You will need the following results
