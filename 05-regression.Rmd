...
*Ex ante forecasts* are those that are made using only the information that is available in advance. For example, ex-ante forecasts for the percentage change in US consumption for 2016Q4 (the quarter following the end of the sample) should only use information that was available *upto and before* 2016Q3. These are genuine forecasts, made in advance using whatever information is available at the time. Ex-ante forecasts for 2016Q4 can be obtained by first generating genuine forecasts for each of the predictors for 2016Q4. For example one of the simple methods introduced in Section \@ref(sec-2-methods) can be used or more sophisticated pure time series approached that follow in Chapters \@ref(ExpSm) and \@ref(ARIMA). Alternatively forecasts by some other source such as a government agency can be used.

### Example 

*Ex post forecasts* are those that are made using later information on the predictors. For example, ex post forecasts of consumption for 2016Q4 may use the actual observations of the predictors for 2016Q4, once these have been observed. These are not genuine forecasts, but are useful for studying the behaviour of forecasting models.
...
In this setting the forecaster assumes possible scenarios for the predictor variable that are of interest. For example the US policy maker may want to forecast consumption if there is a 1% growth in each income and savings but no change in unemployment,  for each of the four quarters following the end of the sample. 
...
fit.consBest<-lm(Consumption ~ Income + Savings + Unemployment, data = uschange)
newdata=data.frame(cbind(Income=c(1,1,1,1),Savings=c(1,1,1,1),Unemployment=c(0,0,0,0)))
...
autoplot(fcast, ylab="% change in consumption", xlab="% change in income")
...
### Building a predictive regression {-}
