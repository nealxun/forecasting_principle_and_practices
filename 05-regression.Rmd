...
Although the linear relationship assumed so far in this chapter is often adequate, there are cases for which a nonlinear functional form is more suitable. To keep things simple in this section suppose we have only one predictor $x$. 
...
Recall that in order to perform a logarithmic transformation to a variable, all its observed values must be greater than zero. In the case that variable $x$ contain zeros we use the transformation $\log(x+1)$. That is, we add one to the value of the variable and then take logarithms. This has a similar effect to taking logarithms but avoids the problem of zeros. It also has the neat side-effect of zeros on the original scale remaining zeros on the transformed scale. 

There are cases for which simply transforming your data will not be adequate and a more general specification may be required. Then the model we use is
$$y=f(x)+\varepsilon$$
...
One of the simplest specifications is to make $f$ piecewise linear. That is, we introduce points where the slope of $f$ can change. These points are called "knots". This can be achieved by leting $x_{1,t}=x$ and introducing variable $x_{2,t}=x$ such that 
\begin{align*}
  x_{1,t} & = x \\ 
  x_{2,t} = (x-c)_+ &= \left\{ 
             \begin{array}{ll} 
             ...
             \end{array}\right.
\end{align*}
The notation $(x-c)_+$ means the value $x-c$ if it is positive and 0 otherwise. This forces the slope to bend at point $c$. Additional bends can be included in the relationship by adding further variables of the above form. An example of this follows by considering $x=t$ and fitting a piecewise linear trend to a times series. 
...
$$ x_{1}= x \quad x_{2} = (x-c_{1})_+ \quad\ldots\quad x_{k} = (x-c_{k-1})_+$$
...
In Section \@ref(Regr-UsefulPredictors) fitting a linear trend to a time series by setting $x=t$ was introduced. However, there are many cases for which a non-linear trend may be more suitable. An example of a non-linear trend is using quadratic or higher order trends  obtained by specifying 
\[
...
A better approach is to use the piecewise specification introduced above and fit a piecwise linear trend which bends at some time. We can think of this as a nonlinear trend constructed of linear pieces. If the trend bends at time $\tau$, then it can be specified by simply replacing $x=t$ and $c=\tau$ above such that we include the predictors,
\begin{align*}
  x_{1,t} & = t \\ 
  x_{2,t} = (t-\tau)_+ &= \left\{ 
             \begin{array}{ll} 
...
dat <- read.csv("Marathon.csv", header=TRUE, sep=",")
times<-window(ts(dat[,2], freq=1,start=dat[1,1]))
fit.lin<-tslm(times~trend)

p1<-autoplot(times)+
  autolayer(fitted(fit.lin),series = "Linear trend")+ 
  xlab("Year") +  ylab("Finishing times in minutes")+
  guides(colour=guide_legend(title=" "))+ 
...
The fitted exponential trend and forward projections from this are shown in Figure \@ref(fig:marathonNonlinear) below. The plot shows that in comparison to the linear trend fitting an exponential trend seems to give a more sensible projection in that the winning times with decrease in the future but in an exponentially decaying rate rather than a linear non-changing rate. 

Simply eyeballing the winning times seems to reveal three very different periods. There seems a lot of volatility in the winning times from the beginning upto aobout 1940 with the winning times decreasing overall but with a significant increase in the decade of the 1920s.

```{r marathonNonlinear, echo=TRUE, message=TRUE, warning=FALSE}
setwd("C:/George/FPPLocalOnly")
dat <- read.csv("Marathon.csv", header=TRUE, sep=",")
times<-window(ts(dat[,2], freq=1,start=dat[1,1]))
fit.lin<-tslm(times~trend)
fit.exp<-tslm(times~trend,lambda = 0)
h=10
...
fit.spline=tslm(times~t+I(t^2)+I(t^3)+I(t1^3)+I(t2^3))
...
autoplot(times)+
  ...
  autolayer(fcasts.pw$mean,series="Piece-wise") +
...
deal with a variety of non-linear forms, and in Section \@ref(sec-5-2-UsefulPredictors) we showed how to include a piecewise-linear
