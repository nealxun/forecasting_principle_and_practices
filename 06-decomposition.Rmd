...
In Section \@ref(tspatterns) we discussed three types of time series patterns: trend, seasonality and cycles. When we decompose a time series into components, we usually combine the trend and cycle into a single "trend-cycle" component which we call the "trend" for simplicity.

Thus we think of a time series as comprising three components: a trend component (containing both trend and cycle), a seasonal component, and a remainder component (containing anything else in the time series). We call the remainder component, the "irregular" component.

In this chapter, we consider some common  methods to extract these components from a time series. Often this is done to help understand the time series better, but it can also be used to improve forecasts.
...
$$
  y_{t} = S_{t} + T_{t} + I_{t},
$$ 
where $y_{t}$ is the data at period $t$, $S_{t}$ is the seasonal component at period $t$, $T_{t}$ is the trend component at period $t$ and $I_{t}$ is the irregular component at period $t$. Alternatively, a multiplicative model would be written as
$$
  y_{t} = S_{t} \times T_{t} \times I_{t}.
$$

The additive model is most appropriate if the magnitude of the seasonal fluctuations or the variation around the trend does not vary with the level of the time series. When the variation in the seasonal pattern, or the variation around the trend, appears to be proportional to the level of the time series, then a multiplicative model is more appropriate. With economic time series, multiplicative models are common.
...
$$  
  y_{t} = S_{t} \times T_{t} \times I_{t} \quad\text{is equivalent to}\quad
  \log y_{t} = \log S_{t} + \log T_{t} + \log I_{t}.
$$
...
```{r elecequip-trend,  fig.cap="Electrical equipment orders: the trend component (red) and raw data (grey).", echo=TRUE, cache=TRUE}
...
Figure \@ref(fig:elecequip-trend) shows the trend component, $T_t$, in red and the original data, $y_t$, in grey. The trend shows the overall movement in the series, ignoring the seasonality and any small random fluctuations.
...
Seasonally adjusted series contain the remainder component as well as the trend. Therefore they are not "smooth" and "downturns" or "upturns" can be misleading. If the purpose is to look for turning points in the series, and interpret any changes in the series, then it is better to use the trend component rather than the seasonally adjusted data.

##Moving averages

The classical method of time series decomposition originated in the 1920s and was widely used until the 1950s. It still forms the basis of many time series decomposition methods, and so it is important to understand how it works. The first step in a classical decomposition is to use a moving average method to estimate the trend, so we begin by discussing moving averages.
...
```
Notice how the trend (in red) is smoother than the original data and captures the main movement of the time series without all the minor fluctuations. The moving average method does not allow estimates of $T_{t}$ where $t$ is close to the ends of the series; hence the red line does not extend to the edges of the graph on either side. Later we will use more sophisticated methods of trend estimation which do allow estimates near the endpoints.

The order of the moving average determines the smoothness of the trend estimate. In general, a larger order means a smoother curve. Figure \@ref(fig:ressales3) shows the effect of changing the order of the moving average for the residential electricity sales data.
...
### Estimating the trend with seasonal data {-}

The most common use of centered moving averages is in estimating the trend from seasonal data. Consider the $2\times4$-MA:
$$
...
In general, a $2\times m$-MA is equivalent to a weighted moving average of order $m+1$ with all observations taking weight $1/m$ except for the first and last terms which take weights $1/(2m)$. So if the seasonal period is even and of order $m$, use a $2\times m$-MA to estimate the trend. If the seasonal period is odd and of order $m$, use a $m$-MA to estimate the trend cycle. For example, a $2\times 12$-MA can be used to estimate the trend of monthly data and a 7-MA can be used to estimate the trend of daily data.

Other choices for the order of the MA will usually result in trend estimates being contaminated by the seasonality in the data.
...
Figure \@ref(fig:elecequip2) shows a $2\times12$-MA applied to the electrical equipment orders index. Notice that the smooth line shows no seasonality; it is almost the same as the trend shown in Figure \@ref(fig:elecequip-trend) which was estimated using a much more sophisticated method than moving averages. Any other choice for the order of the moving average (except for 24, 36, etc.) would have resulted in a smooth line that shows some seasonal fluctuations.
...
A major advantage of weighted moving averages is that they yield a smoother estimate of the trend. Instead of observations entering and leaving the calculation at full weight, their weights are slowly increased and then slowly decreased resulting in a smoother curve.
...
Step 1
:   If $m$ is an even number, compute the trend component using a $2\times m$-MA to obtain $\hat{T}_t$. If $m$ is an odd number, compute the trend component using an $m$-MA to obtain $\hat{T}_t$.
...
Step 4
:   The remainder component is calculated by subtracting the estimated seasonal and trend components: $\hat{E}_t = y_t - \hat{T}_t - \hat{S}_t$.
...
Step 4
:   The remainder component is calculated by dividing out the estimated seasonal and trend components: $\hat{E}_t = y_t /( \hat{T}_t  \hat{S}_t)$.
...
1. Compute a $2\times12$ moving average applied to the original data to obtain a rough estimate of the trend $\hat{T}_t$ for all periods.
...
9. The trend $\hat{T}_t$ is estimated by applying a weighted Henderson MA to the preliminary seasonally adjusted values. (The greater the randomness, the longer the length of the moving average used.) For monthly series: either a 9-, 13-, or 23-term Henderson moving average is used.
...
14. The remainder component is obtained by dividing the seasonally adjusted data from Step 13 by the trend obtained in Step 9.

15. Extreme values of the remainder component are replaced as in Step 5.

16. A series of modified data is obtained by multiplying the trend, seasonal component, and adjusted remainder component together.
...
- The smoothness of the trend can also be controlled by the user.

- It can be robust to outliers (i.e., the user can specify a robust decomposition). So occasional unusual observations will not affect the estimates of the trend and seasonal components. They will, however, affect the remainder component.
...
(b) Use a classical multiplicative decomposition to calculate the trend and seasonal indices.
