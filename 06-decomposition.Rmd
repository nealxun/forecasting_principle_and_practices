...
fit <- stl(elecequip, s.window=7, t.window=13) 
...
One of the most popular methods for decomposing quarterly and monthly data is X-13-ARIMA-SEATS, which has its origins in methods developed by the US Bureau of the Census and the Bank of Spain. It is now widely used by government agencies around the world. 

The X-13-ARIMA-SEATS method is based on classical decomposition, but includes many extra steps and features in order to overcome the drawbacks of classical decomposition that were discussed in the previous section. In particular, trend-cycle estimates are available for all observations including the end points, and the seasonal component is allowed to vary slowly over time. It is also relatively robust to occasional unusual observations. X-13-ARIMA-SEATS also has some sophisticated methods for handling trading day variation, holiday effects and the effects of known predictors. It handles both additive and multiplicative decomposition.

The “ARIMA” part of X-13-ARIMA-SEATS refers to the use of an ARIMA model (see Chapter \@ref(ch-arima)) which provides forecasts of the series forward in time, as well as backwards in time. Then, when a moving average is applied to obtain an estimate of the trend-cycle, there is no loss of observations at the start and end of the series.

The "SEATS" part of the name stands for "Seasonal Extraction in ARIMA Time Series".

The details of the X-13-ARIMA-SEATS method are beyond the scope of this book. However, a complete discussion of the method is available in @Dagum2016. Here we will only demonstrate how to use the automatic procedure in R.


### The `seasonal` package for R {-}

The X-13-ARIMA-SEATS method is implemented in the `seasonal` package for R.

```{r x13arima, echo=TRUE, warning=FALSE, cache=TRUE, fig.width=8, fig.height=7, fig.cap="An X-13-ARIMA-SEATS decomposition of the new orders index for electrical equipment."}
library(seasonal)
fit <- seas(elecequip) 
autoplot(fit) +
  ggtitle("X-13-ARIMA-SEATS decomposition of electrical equipment index")
```
...
The `seasonal` package has many options for handling variations of X-13-ARIMA-SEATS. See [the package website](http://www.seasonal.website/seasonal.html) for a detailed introduction to the options and features available.
...
As with the other decomposition methods discussed in this book, to obtain the separate components plotted in Figure \@ref(fig:classical-elecequip), use the `seasonal()` function for the seasonal component, the `trendcycle()` function for trend-cycle component, and the `remainder()` function for the remainder component. 
