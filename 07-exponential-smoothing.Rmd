...
```{r fig-7-LevelTrendSeas, fig.cap="Estimated components for the Holt-Winters method with additive and multiplicative seasonal components.", echo=FALSE}
...
```{r MAMstates, fig.cap="Graphical representation of the estimated states over time.", echo=TRUE}
autoplot(fit)
```
...
```{r MAMforecasts, fig.cap="Forecasting international visitor nights in Australia using an ETS(M,A,M) model.", echo=TRUE}
fit %>% forecast(h=8) %>%
  autoplot() +
