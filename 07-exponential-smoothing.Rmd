...
```{r 7-oil, fig.cap="Oil production in Saudi Arabia from 1996 to 2007.", echo=TRUE, cache=TRUE}
...
```{r ses, fig.cap="Simple exponential smoothing applied to oil production in Saudi Arabia (1996--2007).", echo=TRUE, cache=TRUE}
fit <- ses(oildata, h=3) 
...
```{r fig_7_trend, fig.cap="Forecasting Air Passengers in Australia (thousands of passengers). For all methods $\\alpha=0.8$ and $\\beta^*=0.0001$, and for the additive damped trend method $\\phi=0.85$.", echo=TRUE, cache=TRUE}
...
```{r fig-7-leveltrend, fig.cap="Level and slope components for Holt’s linear trend method and the additive damped trend method.", echo=TRUE, cache=TRUE}
...
```{r 7-HW, fig.cap="Forecasting international visitor nights in Australia using Holt-Winters method with both additive and multiplicative seasonality.", echo=TRUE, cache=TRUE}
...
```{r fig-7-LevelTrendSeas, fig.cap="Estimated components for Holt-Winters method with additive and multiplicative seasonal components.", echo=FALSE, cache=TRUE}
...
```{r, 7-oil-b, fig.cap="Point forecasts and 80% and 95% prediction intervals from model ETS(A,N,N).", echo=TRUE, cache=TRUE}
oildata <- window(oil,start=1996,end=2007) 
...
```{r MAMforecasts, fig.cap="Forecasting international visitor nights in Australia from an ETS(M,A,M) model.", echo=TRUE, cache=TRUE}
