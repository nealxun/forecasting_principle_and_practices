...
```{r 7-oil, fig.cap="Oil production in Saudi Arabia from 1980 to 2013.", echo=TRUE, cache=FALSE}
...
```{r alpha, echo=FALSE, cache=FALSE}
...
Unlike the regression case (where we have formulae which return the values of the regression coefficients that minimize the SSE), this involves a non-linear minimization problem, and we need to use an optimization tool to solve it.
...
```{r sesfit, echo=TRUE, cache=FALSE}
oildata <- window(oil, start=1996)
# Estimate parameters
fc <- ses(oildata, h=5)
...
```{r oilses, echo=FALSE, cache=FALSE}
# Data set for table
x <- oildata
# Generate forecasts
fc <- ses(x, h=3)
...
# Show table
knitr::kable(tab, caption="Forecasting the total oil production in millions of tonnes for Saudi Arabia using simple exponential smoothing.")
```

The black line in Figure \@ref(fig:ses) is a plot of the data, which shows a changing level over time.


```{r ses, fig.cap="Simple exponential smoothing applied to oil production in Saudi Arabia (1996--2013).", echo=TRUE, cache=FALSE}
autoplot(fc) +
...
```{r airholt0, echo=TRUE, cache=FALSE, fig.cap="Total annual passengers of air carriers registered in Australia. 1990-2014."}
...
```{r airholt1, echo=TRUE, cache=FALSE}
fc <- holt(air, h=5)
```

```{r airholt, echo=FALSE, cache=FALSE}
# Now set up table
...
```{r checkholt, echo=FALSE, cache=FALSE}
...
```{r dampedtrend, fig.cap="Forecasting Air Passengers in Australia (millions of passengers). For the damped trend method, $\\phi=0.90$.", echo=TRUE, cache=FALSE}
fc <- holt(air, h=15)
fc2 <- holt(air, damped=TRUE, phi = 0.9, h=15)
autoplot(air, series="Data") +
...
```{r sheep, fig.cap="Annual sheep livestock numbers in Asia (in million head)", echo=TRUE, cache=FALSE}
autoplot(livestock) +
...
```{r expsmoothcv, echo=TRUE, cache=FALSE}
...
```{r fig-7-comp, fig.cap="Forecasting livestock, sheep in Asia: comparing forecasting performance of non-seasonal method.", echo=TRUE, cache=FALSE}
autoplot(fc) +
  xlab("Year") + ylab("Livestock, sheep in Asia (millions)") +
  fppcols()
```
...
```{r 7-HW, fig.cap="Forecasting international visitor nights in Australia using the Holt-Winters method with both additive and multiplicative seasonality.", echo=TRUE, cache=FALSE}
aust <- window(austourists,start=2005)
fit1 <- hw(aust,seasonal="additive")
fit2 <- hw(aust,seasonal="multiplicative")
autoplot(aust, series="Data") +
...
```{r tab75, echo=FALSE, cache=FALSE}
# Set up matrix
...
```{r fig-7-LevelTrendSeas, fig.cap="Estimated components for the Holt-Winters method with additive and multiplicative seasonal components.", echo=FALSE, cache=FALSE}
...
The R code below shows all the possible arguments this function takes, and their default values. If only the time series is specified, and all other arguments are left at their default values, then an appropriate model will be selected automatically. We explain the most important arguments below. See the help file for a more complete description.

```r
ets(y, model="ZZZ", damped=NULL, alpha=NULL, beta=NULL, gamma=NULL,
    phi=NULL, lambda=NULL, biasadj=FALSE,
    ic=c("aicc","aic","bic"),
    additive.only=FALSE, restrict=TRUE, allow.multiplicative.trend=FALSE)
```
...
To obtain forecasts from an ETS model, we use the `forecast` function. The R code below shows the all the possible arguments this function takes when applied to an ETS model. We explain each the arguments in what follows.
...
The parameter estimates are $\alpha=`r format(fit$par[1],nsmall=4,digits=4)`$, $\beta=`r format(fit$par[2],nsmall=4,digits=4, scientific=FALSE)`$, and $\gamma=`r format(fit$par[3],digits=4,nsmall=4, scientific=FALSE)`$. The output also returns the estimates for the initial states $\ell_0$, $b_0$, $s_{0}$, $s_{-1}$, $s_{-2}$ and $s_{-3}.$ The training RMSE for this model is slightly lower than the Holt-Winters methods with additive and multiplicative seasonality presented in Tables \@ref(tbl-7-HWadd) and \@ref(tbl-7-HWmulti) respectively. Although this model gives equivalent point forecasts to a multiplicative Holt-Winters' method, the parameters have been estimated differently. With the `ets()` function, the default estimation method is maximum likelihood rather than minimum sum of squares.
...
```{r MAMstates, fig.cap="Graphical representation of the estimated states over time.", echo=TRUE, cache=FALSE}
autoplot(fit)
```

```{r MAMforecasts, fig.cap="Forecasting international visitor nights in Australia from an ETS(M,A,M) model.", echo=TRUE, cache=FALSE}
