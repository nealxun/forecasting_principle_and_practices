...
R uses a slightly different parameterization to that shown above.  Rather than \@ref(eq-8-arima), it uses
$$\label{eq-R-arima}
  (y'_{t}-\mu) = \phi_{1}(y'_{t-1}-\mu) + \cdots + \phi_{p}(y'_{t-p}-\mu)
     + \theta_{1}e_{t-1} + \cdots + \theta_{q}e_{t-q} + e_{t},
$$
where $\mu = c/(1-\phi_1 - \cdots - \phi_p)$ is an estimate of the mean of the series $\{y'_t\}$.
...
The process is summarized in Figure \@ref(fig:diagram).

```{r diagram, cache=FALSE}
```
