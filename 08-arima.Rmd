...
3. For your retail data (Exercise 1 of Section \@ref(ex:graphics)), find the appropriate order of differencing (after transformation if necessary) to obtain stationary data.
...
8. Consider the total net generation of electricity (in billion kilowatt hours) by the U.S. electric industry (monthly for the period 1985--1996). (Data set `usmelec`.) In general there are two peaks per year: in mid-summer and mid-winter.
    ...
    (f) Forecast the next 15 years of electricity generation by the U.S. electric industry. Get the latest figures from <http://data.is/zgRWCO> to check the accuracy of your forecasts.
...
11. For the same time series you used in the previous exercise, try using a non-seasonal model applied to the seasonally adjusted data obtained from STL. The `stlf()` function will make the calculations easy (with `method="arima"`). Compare the forecasts with those obtained in the previous exercise. Which do you think is the best approach?

12. For your retail time series (Exercise 5 above):
