...
```{r acfstationary, fig.cap="The ACF of the Dow-Jones index (left) and of the daily changes in the Dow-Jones index (right)."}
```
...
```{r a10diff, fig.cap="Logs and seasonal differences of the A10 (antidiabetic) sales data. The logarithms stabilize the variance, while the seasonal differences remove the seasonality and trend."}
autoplot(a10) + xlab("Year") + 
  ylab("Sales ($million)") +
  ggtitle("Antidiabetic drug sales") 
autoplot(log(a10), xlab="Year", ylab="Monthly log sales")
autoplot(diff(log(a10),12), xlab="Year", ylab="Annual change in monthly log sales")
```
...
```{r arp, fig.cap="Two examples of data from autoregressive models with different parameters. Left: AR(1) with $y_t = 18 -0.8y_{t-1} + e_t$. Right: AR(2) with $y_t = 8 + 1.3y_{t-1}-0.7y_{t-2}+e_t$. In both cases, $e_t$ is normally distributed white noise with mean zero and variance one."}
```
...
```{r maq, fig.cap="Two examples of data from moving average models with different parameters. Left: MA(1) with $y_t = 20 + e_t + 0.8e_{t-1}$. Right: MA(2) with $y_t = e_t- e_{t-1}+0.8e_{t-2}$. In both cases, $e_t$ is normally distributed white noise with mean zero and variance one."}
...
```{r usconsumptionf, fig.cap="Forecasts of quarterly percentage change in US consumption expenditure."}
...
```{r usconsumptionacf, fig.cap="ACF and PACF of quarterly percentage change in US consumption. A convenient way to produce a time plot, ACF plot and PACF plot in one command is to use the `tsdisplay` function in R."}
par(mfrow=c(1,2)) 
...
$$
  \AICc = \text{AIC} + \frac{2(p+q+k+1)(p+q+k+2)}{T-p-q-k-2}.
$$
...
```{r ee2, fig.cap="Time plot and ACF and PACF plots for differenced seasonally adjusted electrical equipment data."}
...
\hspace*{1.2cm} ${\uparrow}$ \hspace{1.2cm} ${\uparrow}$ \\
\hspace*{.2cm}{$\left(\text{\begin{tabular}{l}
Non-seasonal part\\ of the model \end{tabular}}\right)$}
\hspace*{.2cm}{$\left(\text{\begin{tabular}{l}
Seasonal part\\ of the model\end{tabular}}\right)$}
\end{tabular}
...
```{r euretail1, fig.cap="Quarterly retail trade index in the Euro area (17 countries), 1996--2011, covering wholesale and retail trade, and repair of motor vehicles and motorcycles. (Index: 2005 = 100)."}
...
Figure \@ref(wnacf) shows the ACFs for 36 random numbers, 360 random numbers and for 1,000 random numbers.
...
\@ref(exercise-Q1) Choose one of the following seasonal time series: `condmilk`, `hsales`, `uselec`
...
For the same time series you used in \@ref(exercise-Q1), try using a non-seasonal model applied to the seasonally adjusted data obtained from STL. The `stlf()` function will make the calculations easy (with `method="arima"`). Compare the forecasts with those obtained in \@ref(exercise-Q1). Which do you think is the best approach?
