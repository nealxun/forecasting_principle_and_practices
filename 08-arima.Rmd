...
```{r djlb, echo=TRUE, cache=FALSE}
Box.test(diff(dj), lag=10, type="Ljung-Box")
```

```{r djlb2, echo=FALSE, cache=FALSE}
...
```{r usmelec, fig.height=6, cache=FALSE, fig.cap="Top panel: US net electricity generation (billion kWh). Other panels show the same data after transforming and differencing."}
tmp <- cbind(
  "Billion kWh" = usmelec,
  "Logs" = log(usmelec),
...
```{r usconsumptionf, fig.cap="Forecasts of quarterly percentage changes in US consumption expenditure.", cache=FALSE}
...
```{r ee2, fig.cap="Time plot and ACF and PACF plots for the differenced seasonally adjusted electrical equipment data.", cache=FALSE}
...
```{r euretail1, fig.cap="Quarterly retail trade index in the Euro area (17 countries), 1996--2011, covering wholesale and retail trade, and the repair of motor vehicles and motorcycles. (Index: 2005 = 100).", cache=FALSE}
...
```{r h02aicc, echo=FALSE, cache=FALSE}
models <- rbind(
  c(3,0,0,2,1,0),
  c(3,0,1,2,1,0),
  c(3,0,2,2,1,0),
...
```{r checkclaimh02, echo=FALSE, cache=FALSE}
if(aicc[6] > min(aicc))
  stop("Not best model")
...
Forecasts from the ARIMA(3,0,1)(0,1,2)$_{12}$ model (which has the lowest RMSE value on the test set, and the best AICc value amongst models with only seasonal differencing) are shown in Figure \@ref(h02f).
...
1. Figure \@ref(fig:wnacfplus) shows the ACFs for 36 random numbers, 360 random numbers and 1,000 random numbers.
    ...
    ```{r wnacfplus, fig.height=3, echo=FALSE, cache=FALSE, fig.cap="Left: ACF for a white noise series of 36 numbers. Middle: ACF for a white noise series of 360 numbers. Right: ACF for a white noise series of 1,000 numbers."}
    x1 <- rnorm(36)
    x2 <- rnorm(360)
    x3 <- rnorm(1000)
...
2. A classic example of a non-stationary series is the daily closing IBM stock price series (data set `ibmclose`). Use R to plot the daily closing prices for IBM stock and the ACF and PACF. Explain how each plot shows the series is non-stationary and should be differenced.
