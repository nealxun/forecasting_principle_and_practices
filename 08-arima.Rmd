...
j <- order(aicc)
knitr::kable(data.frame(Model=modelname,AICc=aicc)[j,],
             digits=2, row.names=FALSE, align='cc')
```
...
modelname <- sub("\\[12\\]","$_{12}$",modelname)
j <- order(rmse)
knitr::kable(data.frame(Model=modelname,RMSE=rmse)[j,],
             digits=4, row.names=FALSE, align='cc')
```
...
11. For the same time series you used in Exercise 10, try using a non-seasonal model applied to the seasonally adjusted data obtained from STL. The `stlf()` function will make the calculations easy (with `method="arima"`). Compare the forecasts with those obtained in Exercise 10. Which do you think is the best approach?

12. For your retail time series (Exercise 4 above):
