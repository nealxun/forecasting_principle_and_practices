...
(fit <- auto.arima(usconsumption[,"consumption"], xreg=usconsumption[,"income"]))
```
...
slope <- coef(fit)['usconsumption[, "income"]']
sigma2 <- fit$sigma2
```
...
fcast <- forecast(fit, xreg=rep(mean(usconsumption[,2]),8))
...
1.  Consider monthly sales and advertising data for an automotive parts company (data set `advert`).
    a. Fit a standard regression model $y_t = a + b x_t + n_t$ where $y_t$ denotes sales and $x_t$ denotes advertising using the `lm()` function.
...
and $y^*_t = \log(Y_t)$, $x^*_{1,t} = \sqrt{x_{1,t}}$ and $x^*_{2,t}=\sqrt{x_{2,t}}$.
  ...
  e. Explain why the $N_t$ term should be modeled with an ARIMA model rather than modeling the data using a standard regression package.  In your discussion, comment on the properties of the estimates, the validity of the standard regression results, and the importance of the $N_t$ model in producing forecasts.
