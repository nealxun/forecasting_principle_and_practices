...
```{r usconsump2, fig.cap="Residuals ($e_t$) obtained from a regression of change in consumption expenditure on change in disposable income, assuming an ARIMA(1,0,2) error model.", cache=TRUE}
(fit <- auto.arima(uschange[,"Consumption"], xreg=uschange[,"Income"]))
```

```{r usconsumpparam, echo=FALSE, cache=TRUE}
phi1 <- coef(fit)['ar1']
theta1 <- coef(fit)['ma1']
theta2 <- coef(fit)['ma2']
...
```{r usconsumpres, fig.cap="Regression errors ($n_t$) and ARIMA errors ($e_t$) from the fitted model.", cache=TRUE}
...
```{r usconsumpres2, fig.cap="The residuals (i.e., the ARIMA errors) are not significantly different from white noise.", cache=TRUE}
checkresiduals(fit)
```
...
```{r deterministictrend, cache=TRUE}
trend <- seq_along(austa)
(fit1 <- auto.arima(austa, d=0, xreg=trend))
```

```{r austaparams, cache=TRUE, echo=FALSE}
phi1 <- coef(fit1)['ar1']
phi2 <- coef(fit1)['ar2']
...
```{r austaf, fig.cap="Forecasts of annual international visitors to Australia using a deterministic trend model and a stochastic trend model.", cache=TRUE, message=FALSE}
...
```{r tvadvert, fig.cap="Numbers of insurance quotations provided per month and the expenditure on advertising per month.", cache=TRUE}
...
```{r tvadvert2, fig.cap="ARIMA models for insurance quotations with advertising as a predictor variable.", cache=TRUE}
...
```{r tvadvertparam, echo=FALSE, cache=TRUE}
...
```{r tvadvertf8, fig.cap="Forecasts of monthly insurance quotes, assuming that the future advertising expenditure is 8 units in each future month.", cache=TRUE}
