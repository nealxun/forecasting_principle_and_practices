...
fit <- auto.arima(calls, seasonal=FALSE, lambda=0,
         xreg=fourier(calls, K=c(30,30)))
fc <- forecast(fit, xreg=fourier(calls, K=c(30,30), h=2*169))
autoplot(fc, include=5*169) +
  ylab("Call volume") + xlab("Weeks")
```

This is a very large model (containing 120+??=`r length(coef(fit))` parameters).
...
```{r elecdemand, cache=TRUE, echo=TRUE}
...
```{r elecdemand2, cache=TRUE, echo=TRUE}
elecdemand %>%
  as.data.frame %>%
...
```{r elecdemand3, cache=TRUE, echo=TRUE}
