...
if(length(coef(bestfit)) - 2L*besti != 3L)
...
If $\hat{q}_{j+1|j}$ and $\hat{a}_{j+1|j}$ are the one-step forecasts of the $(j+1)$th demand size and inter-arrival time respectively, based on data up to demand $j$, then Croston's method gives
\begin{align}
\hat{q}_{j+1|j} & = (1-\alpha)\hat{q}_{j|j-1} + \alpha q_j,\label{c2method1}\\
\hat{a}_{j+1|j} & = (1-\alpha)\hat{a}_{j|j-1} + \alpha a_j.\label{c2method2}
\end{align}
The smoothing parameter $\alpha$ takes values between 0 and 1 and
is assumed to be the same for both equations. Let $\ell
= j_n$ denote the last period of demand. Then the mean demand
rate, which is used as the $h$-step ahead forecast for the demand
at time $n+h$, is estimated by the ratio
\begin{equation}\label{c2ratio}
\hat{y}_{T+h|T} = q_{\ell}/a_{\ell}.
\end{equation}
...
What to do about these issues depends on the purpose of the model. A more flexible and complicated model could be used, but this still assumes that the model structure will work over the whole period of the data. A better approach is usually to allow the model itself to change over time. ETS models are designed to handle this situation by allowing the trend and seasonal terms to evolve over time. 

If you are only interested in forecasting the next few observations, it is roughly equivalent and simpler to throw away the earliest observations and only fit a model to the most recent observations.  

For example, we fitted a dynamic harmonic regression model to 26 years of weekly gasoline production in Section \@ref(weekly). It is, perhaps, unrealistic to assume that the seasonal pattern remains the same over nearly three decades. So we can simply fit a model to the most recent years instead.

```{r gasrecent}
recentgas <- window(gasoline, start=2007)
bestfit <- list(aicc=Inf)
for(i in seq(25))
{
  fit <- auto.arima(gasoline, xreg=fourier(gasoline, K=i), seasonal=FALSE)
...
## Out-of-sample one-step forecasts {#oosos}

## In-sample multi-step forecasts {#isms}
